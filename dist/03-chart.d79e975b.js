...
module.exports = "/all-temps.837d9c2a.csv";
...
function ready(datapoints) {
  ...
  var tucsonDatapoints = datapoints.filter(function (d) {
  ...
  var melbourneDatapoints = datapoints.filter(function (d) {
    return d.city === 'Melbourne';
  });
  ...
  container.append('path').attr('class', 'temp').datum(nycDatapoints).attr('d', line).attr('fill', 'black').attr('opacity', 0.75);
  ...
  d3.select('#nyc').on('stepin', function () {
    container.selectAll('.temp').datum(nycDatapoints).transition().attr('d', line).attr('fill', function (d) {
      return colorScale(nycMean);
    });
    d3.selectAll('.label-NYC').style('background', function (d) {
      return colorScale(nycMean);
    }).style('color', 'white');
  ...
  d3.select('#beijing').on('stepin', function () {
    container.selectAll('.temp').datum(beijingDatapoints).transition().attr('d', line).attr('fill', function (d) {
      return colorScale(beijingMean);
    });

    d3.selectAll('.label-Beijing').style('background', function (d) {
      return colorScale(beijingMean);
    }).style('color', 'white');
  ...
  d3.select('#stockholm').on('stepin', function () {
    container.selectAll('.temp').datum(stockholmDatapoints).transition().attr('d', line).attr('fill', function (d) {
      return colorScale(stockholmMean);
    });

    d3.selectAll('.label-Stockholm').style('background', function (d) {
      return colorScale(stockholmMean);
    }).style('color', 'white');
  ...
  d3.select('#lima').on('stepin', function () {
    container.selectAll('.temp').datum(limaDatapoints).transition().attr('d', line).attr('fill', function (d) {
      return colorScale(limaMean);
    });

    d3.selectAll('.label-Lima').style('background', function (d) {
      return colorScale(limaMean);
    }).style('color', 'white');
  ...
  d3.select('#tucson').on('stepin', function () {
    container.selectAll('.temp').datum(tucsonDatapoints).transition().attr('d', line).attr('fill', function (d) {
      return colorScale(tucsonMean);
    });

    d3.selectAll('.label-Tucson').style('background', function (d) {
      return colorScale(tucsonMean);
    }).style('color', 'white');
  ...
  function render() {
    ...
    var screenWidth = width / height * screenHeight;

    var side = Math.min(screenHeight, screenWidth);

    var newWidth = side - margin.left - margin.right;
    var newHeight = side - margin.top - margin.bottom;
    ...
    svg.selectAll('.temp-notes').attr('y', function (d) {
      return -radiusScale(d);
    });
...
},{"d3":"../node_modules/d3/index.js","./data/all-temps.csv":"data/all-temps.csv"}],"../../../../.npm-global/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
//# sourceMappingURL=/03-chart.d79e975b.map